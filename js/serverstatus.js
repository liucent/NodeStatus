...
function uptime() {
    $.getJSON(find_uri, function (result) {
        ...
        for (var i = 0; i < result.servers.length; i++) {
            ...
            if (!result.servers[i].online4 && !result.servers[i].online6) {
                ...
                if (TableRow.children["edit"].children["delete"]) {
                    TableRow.children["edit"].removeChild(TableRow.children["edit"].children["delete"]);
                }
            ...
            if (result.servers[i].updated) {
                server_updated[i] = result.servers[i].updated;
            } else {
                server_updated[i] = 0;
            }
        }
...
function updateTime() {
    if (!error)
        $("#updated").html("Last updated: " + timeSince(new Date(Math.max(...server_updated) * 1000)));
}
